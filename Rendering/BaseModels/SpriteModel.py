## Code "stolen" from Snake Engine 2 - GraphicsEnginePipeline.RendererCore.py
...
class Sprite:
    ...
    def __init__(self, img_src, size, position=Vector2(), partial=False, part=Rectangle2()):
        self.__size = size
    ...
    def set_size(self, size):
        ...
        g.modifie_taille_image(self.get_img_src(), int(self.__size.x), int(self.__size.y))
    ...
    def _set_sprite_size(self, size):
        ...
        if _image_previous_sizes[self.__img_src].x == size.x and _image_previous_sizes[self.__img_src].y == size.y:
            return
        _image_previous_sizes[self.__img_src] = size.copy()
        g.modifie_taille_image(self.get_img_src(), int(size.x), int(size.y))
    ...
    def set_partial_part(self, partial=True, part=Rectangle2()):
        self.partial = partial
        self.set_part(part)

    def set_part(self, part):
        self._r_part = Rectangle2(part.pos.to_tuple(), (part.pos + part.size).to_tuple())
        self._r_pos = self.__position + part.pos

    def get_part(self):
        return self._r_part.copy()

    def set_to_fit_in_box(self, bounding_box=Rectangle2(), sprite_fit_mode=SpriteFitMode.FIT):
        """
        ...
        self.set_position(bounding_box.pos + Vector2.scalar(0.5))
        self.set_partial_part(False)
        if sprite_fit_mode == SpriteFitMode.STRETCH:
            self.set_size(bounding_box.size + Vector2.scalar(0.5))
            ...
            self.set_position(self.get_position() + (bounding_box.size - self.get_size()) / 2 + Vector2.scalar(0.5))
